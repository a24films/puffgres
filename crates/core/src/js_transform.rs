...
/// file by spawning `npx tsx <script>` as a subprocess. Events are serialized
...
impl Transformer for JsTransformer {
    async fn transform_batch(
        ...
        let mut child = Command::new("npx")
            ...
            .stderr(Stdio::piped())
            .spawn()
            .map_err(|e| CoreError::Pipeline(format!("failed to spawn npx tsx: {e}")))?;
        ...
        let output = child
            .wait_with_output()
            .await
            .map_err(|e| CoreError::Pipeline(format!("failed to wait for npx tsx: {e}")))?;
