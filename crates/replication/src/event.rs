...
///
/// Tuple data is wrapped in `Arc` for zero-copy routing: when the router fans
/// out an event to multiple config pipelines or the DLQ, only the `Arc` pointer
/// is cloned — not the underlying column bytes.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RowEvent {
    pub relation_id: u32,
    pub operation: Operation,
    /// New tuple data (present for Insert and Update).
    pub new_tuple: Option<Arc<TupleData>>,
    /// Old tuple data (present for Delete; present for Update when
    /// REPLICA IDENTITY is FULL or when key columns are sent).
    pub old_tuple: Option<Arc<TupleData>>,
}
