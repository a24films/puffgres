...
fn decode_commit(buf: &mut Bytes) -> Result<WalMessage> {
    need(buf, 25)?;
    Ok(WalMessage::Commit(CommitInfo {
        flags: buf.get_u8(),
...
fn decode_insert(buf: &mut Bytes) -> Result<WalMessage> {
    ...
    Ok(WalMessage::Insert(InsertInfo { relation_id, tuple }))
}
...
fn decode_update(buf: &mut Bytes) -> Result<WalMessage> {
    ...
    Ok(WalMessage::Update(UpdateInfo {
        relation_id,
        old_tuple,
        new_tuple,
    }))
...
fn decode_delete(buf: &mut Bytes) -> Result<WalMessage> {
    ...
    let marker = buf.get_u8();
    match marker {
        b'K' | b'O' => Ok(WalMessage::Delete(DeleteInfo {
            relation_id,
...
fn decode_truncate(buf: &mut Bytes) -> Result<WalMessage> {
    ...
    Ok(WalMessage::Truncate(TruncateInfo {
        option_bits,
        relation_ids,
    }))
}
